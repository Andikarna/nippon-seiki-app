import { T as TSS_SERVER_FUNCTION, c as createServerFn } from "./server-CtKxx6LB.mjs";
...
async function setupTables(p) {
  try {
    ...
    const [partRows] = await p.query("SELECT COUNT(*) as count FROM parts");
...
const getDashboardData = createServerFn({
  ...
  const [lineRows] = await p.query("SELECT id, name, type, active, capacity, shifts, operators FROM production_lines");
...
const getFifoMaterials = createServerFn({
  ...
  const p = await getPool();
  const [rows] = await p.query("SELECT id, part_number as partNumber, lot_number as lotNumber, DATE_FORMAT(incoming_date, '%Y-%m-%d') as incomingDate, position, status, quantity, original_quantity as originalQuantity, origin_line as originLine, operator FROM fifo_materials ORDER BY incoming_date DESC");
  const counts = {
...
const addProductionRecord = createServerFn({
  ...
  await p.query(`INSERT INTO fifo_materials (id, part_number, lot_number, incoming_date, position, status, quantity, original_quantity, origin_line, operator) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE quantity = quantity + ?`, [materialId, partNumber, lotNumber, date, position, status, quantity, quantity, line, operator, quantity]);
...
const getSettingsData = createServerFn({
  ...
  const [lines] = await p.query("SELECT id, name, type, active, capacity, shifts, operators FROM production_lines");
  return {
    users: usersMapped,
...
const addSettingLine = createServerFn({
  ...
  await p.query("INSERT INTO production_lines (id, name, type, capacity, shifts, operators) VALUES (?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE name = ?, type = ?, capacity = ?, shifts = ?, operators = ?", [id, name, type, capacity, shifts, operators, name, type, capacity, shifts, operators]);
  return {
    success: true
  };
});
...
const updateSettingLine = createServerFn({
  ...
  await p.query("UPDATE production_lines SET name = ?, type = ?, capacity = ?, shifts = ?, operators = ? WHERE id = ?", [name, type, capacity, shifts, operators, id]);
  return {
    success: true
  };
});
...
const getActiveLines = createServerFn({
  ...
  const p = await getPool();
  try {
    const [rows] = await p.query("SELECT id, name, type FROM production_lines WHERE active = 1");
    return rows;
  } catch (e) {
...
const getNextLotNumber_createServerFn_handler = createServerRpc({
  id: "c26fd779dda0245bee08ddc5d6ae671875a26ffa00cf133e4a7aa17e4367e6c6",
  name: "getNextLotNumber",
  filename: "src/lib/api/db.functions.ts"
}, (opts) => getNextLotNumber.__executeServer(opts));
const getNextLotNumber = createServerFn({
  method: "POST"
}).inputValidator(objectType({
  line: stringType().min(1),
  date: stringType().min(1)
})).handler(getNextLotNumber_createServerFn_handler, async ({
  data
}) => {
  const p = await getPool();
  const {
  ...
  try {
    const [rows] = await p.query("SELECT lot_number FROM fifo_materials WHERE lot_number LIKE ?", [`${basePattern}%`]);
    const existingLots = rows.map((r) => r.lot_number);
    let seq = 1;
    ...
    return {
      lotNumber: `${basePattern}-1`
    };
  }
});
const getParts_createServerFn_handler = createServerRpc({
  id: "31592cba9d9bc29a292a8b6e3dadf0f111ffd3d86365bc13b918f3418433e807",
  name: "getParts",
  filename: "src/lib/api/db.functions.ts"
}, (opts) => getParts.__executeServer(opts));
const getParts = createServerFn({
  method: "GET"
}).handler(getParts_createServerFn_handler, async () => {
  const p = await getPool();
  try {
    const [rows] = await p.query("SELECT part_number as partNumber, product_name as productName, threshold FROM parts ORDER BY part_number ASC");
    return rows.map((r) => ({
...
const addPart_createServerFn_handler = createServerRpc({
  id: "fd038362282fb52af730a3c94c169732d0e0755940135483815ce0020fb2736d",
  name: "addPart",
  filename: "src/lib/api/db.functions.ts"
...
const addPart = createServerFn({
...
})).handler(addPart_createServerFn_handler, async ({
  data
}) => {
  const p = await getPool();
  const {
  ...
  try {
    await p.query("INSERT INTO parts (part_number, product_name, threshold) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE product_name = ?, threshold = ?", [partNumber.trim(), productName.trim(), threshold, productName.trim(), threshold]);
    return {
      success: true
    };
  } catch (e) {
...
const updatePart_createServerFn_handler = createServerRpc({
  id: "0e6ba0eede7dc731dd9da2f2037e83d2440671e04e2e5a2d327b0d16345e1b39",
  name: "updatePart",
  filename: "src/lib/api/db.functions.ts"
...
const updatePart = createServerFn({
  ...
  productName: stringType().min(1),
  threshold: numberType()
})).handler(updatePart_createServerFn_handler, async ({
  data
}) => {
  const p = await getPool();
  const {
  ...
  try {
    await p.query("UPDATE parts SET product_name = ?, threshold = ? WHERE part_number = ?", [productName.trim(), threshold, partNumber.trim()]);
    return {
      success: true
    };
  } catch (e) {
...
const deletePart_createServerFn_handler = createServerRpc({
  id: "46a4bef42551f766f2114026cb409ae16c0b9f6d9bb3866229829be9dfe3923c",
  name: "deletePart",
  filename: "src/lib/api/db.functions.ts"
}, (opts) => deletePart.__executeServer(opts));
const deletePart = createServerFn({
  method: "POST"
}).inputValidator(objectType({
  partNumber: stringType().min(1)
})).handler(deletePart_createServerFn_handler, async ({
  data
}) => {
  const p = await getPool();
  const {
...
export {
  ...
  deletePart_createServerFn_handler,
