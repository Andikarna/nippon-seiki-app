import { T as TSS_SERVER_FUNCTION, c as createServerFn } from "./server-DvOc5icW.mjs";
...
const getSettingsData = createServerFn({
  ...
  const p = await getPool();
  const [users] = await p.query("SELECT name, email, role, active FROM users");
...
const addSettingUser = createServerFn({
  ...
  await p.query("INSERT INTO users (name, email, role, password) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE name = ?, role = ?, password = ?", [name, email, role.toLowerCase(), pwd, name, role.toLowerCase(), pwd]);
  return {
    success: true
  };
});
...
const updateSettingUser = createServerFn({
  ...
  await p.query("UPDATE users SET name = ?, role = ?, active = ? WHERE email = ?", [name, role.toLowerCase(), active ? 1 : 0, email]);
  return {
    success: true
  };
});
...
const authenticateUser = createServerFn({
  ...
  try {
    const [rows] = await p.query("SELECT name, email, role, password FROM users WHERE LOWER(email) = LOWER(?)", [email.trim()]);
    if (rows.length > 0) {
